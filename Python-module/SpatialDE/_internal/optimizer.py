...
class MultiScipyOptimizer:
    ...
    def minimize(self, method='bfgs', **scipy_kwargs):
        res = scipy.optimize.minimize(self._obj, concat_tensors(self.variables).numpy(), method=method, jac=True, **scipy_kwargs)
    ...
    def _wrap_func(cls, func, vars):
        def _objective(x):
            ...
            with tf.GradientTape() as t:
                obj = func()
            grads = concat_tensors(t.gradient(obj, vars, unconnected_gradients=tf.UnconnectedGradients.ZERO))
            return obj, grads
